...
const App: React.FC = () => {
  ...
  // ... Material update effect - UPDATED TO SPLIT CONCERNS ...
  
  // Texture Transform Updates
  useEffect(() => {
    ...
    // Don't need 'needsUpdate' for uniform changes like repeat/offset if texture exists
  }, [
      params.texture_scale, 
      params.engineering_texture_scale, params.engineering_texture_rotation_offset, 
      params.nacelle_texture_scale, 
      hullMaterial, secondaryMaterial, engineeringMaterial, nacelleMaterial
  ]);

  // Emissive Intensity Updates
  useEffect(() => {
    ...
    neckMaterial.emissive = new THREE.Color('#ffffff');
      ...
      params.texture_emissive_intensity, params.saucer_texture_emissive_intensity, 
      params.bridge_texture_emissive_intensity, params.engineering_texture_emissive_intensity, 
      params.nacelle_texture_glow_intensity, params.neck_texture_glow_intensity,
      hullMaterial, saucerMaterial, bridgeMaterial, engineeringMaterial, nacelleMaterial, neckMaterial
  ]);

  // Texture Toggles (Needs Material Recompile)
  useEffect(() => {
    let changed = false;
    if (!params.texture_toggle) { hullMaterial.map = null; hullMaterial.normalMap = null; hullMaterial.emissiveMap = null; secondaryMaterial.map = null; secondaryMaterial.normalMap = null; changed=true;}
    if (!params.saucer_texture_toggle) { saucerMaterial.map = null; saucerMaterial.normalMap = null; saucerMaterial.emissiveMap = null; changed=true;}
    if (!params.bridge_texture_toggle) { bridgeMaterial.map = null; bridgeMaterial.normalMap = null; bridgeMaterial.emissiveMap = null; changed=true;}
    if (!params.nacelle_texture_toggle) { nacelleMaterial.map = null; nacelleMaterial.normalMap = null; nacelleMaterial.emissiveMap = null; changed=true;}
    if (!params.engineering_texture_toggle) { engineeringMaterial.map = null; engineeringMaterial.normalMap = null; engineeringMaterial.emissiveMap = null; changed=true;}
    if (!params.neck_texture_toggle) { neckMaterial.map = null; neckMaterial.normalMap = null; neckMaterial.emissiveMap = null; changed=true;}
    ...
    if(changed) {
      ...
      params.texture_toggle, params.saucer_texture_toggle, params.bridge_texture_toggle, 
      params.nacelle_texture_toggle, params.engineering_texture_toggle, params.neck_texture_toggle,
      hullMaterial, saucerMaterial, bridgeMaterial, nacelleMaterial, engineeringMaterial, neckMaterial, secondaryMaterial
  ]);
